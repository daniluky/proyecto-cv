...
const generarCVenPDF = (cv, res) => {
    ...
    if (cv.experiencia?.length) {
      ...
      cv.experiencia.forEach(exp => {
        ...
        doc.fillColor('black')
           .font('Helvetica').text(`${new Date(exp.fechaInicio).toLocaleDateString()} - ${new Date(exp.fechaFin).toLocaleDateString()}`);
    ...
    if (cv.educacion?.length) {
      ...
      cv.educacion.forEach(edu => {
        ...
        doc.fillColor('black')
           .font('Helvetica').text(`${new Date(edu.fechaInicio).toLocaleDateString()} - ${new Date(edu.fechaFin).toLocaleDateString()}`);
